                ...
                <div class="w-full">
                    <div class="flex flex-wrap md:justify-between w-full">
                        ...
                        <div class="flex items-end">
                            <button
                                class="mr-1 px-3 py-2 bg-sky-800 rounded-lg text-sm text-white justify-items-center hover:bg-sky-700"
                    ...
                    <TableComponent
                        :headers="['#', 'Penanda Tangan', 'Pembuat Nomor', 'Nomor Naskah', 'Jenis Naskah', 'Klasifikasi', 'Tanggal', 'tujuan', 'perihal', 'Proses By', 'action']"
            ...
            <form @submit.prevent="statusModal ? createNomor() : updateNomor()">
                <div class="grid gap-6 mb-6 md:grid-cols-1 md:w-96">
                    ...
                    <div>
                        ...
                        <Multiselect v-model="form.id_pengguna" :options="users" placeholder="penerima nomor"
                            label="nama_pengguna" track-by="nama_pengguna">

                        </Multiselect>
                        <span v-if="errors.id_pengguna" class="text-red-500">{{ errors.id_pengguna[0]
                            }}</span>
...
const loadNomor = async (page = 1) => {
    ...
    not_found.value = false
    await axios.get(`/api/nomor?page=${page}`, {
        headers: {
            Authorization: "Bearer" + localStorage.getItem("token"),
        }, params: fillter
    }).then((res) => {
    ...
    if (checkTime()) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.post("/api/nomor-admin", {
                        ...
                        id_pengguna: form.value.id_pengguna
...
const deleteNomor = (id) => {
    ...
    Swal.fire({
        title: "Apakah anda yakin membtalkan nomor ini?",
        text: "Pembuat nomor ini tidak akan bisa menggunakannya!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
            ...
            axios.delete("/api/nomor/" + id, {
                ...
                Swal.fire({
                    title: "Dibatalkan!",
                    text: "Nomor ini telah dibatalkan.",
                    icon: "success"
                });
            }).catch((err) => {
                if (
...
const toggleModalUpdate = (item) => {
    ...
    form.value.id_pengguna = item.users
